...
unsigned char eth_type_is_ip_and_my_ip(unsigned char* buf, unsigned  int len)
{
   ...
   if (len < 42)
   {
   	//	syslog(DBG,"recv len < 43");
      return(0);
   }
   if (buf[ETH_TYPE_H_P] != ETHTYPE_IP_H_V ||
      buf[ETH_TYPE_L_P] != ETHTYPE_IP_L_V)
   {
		//	syslog(DBG,"recv type err");
      return(0);
   }
   if (buf[IP_HEADER_LEN_VER_P] != 0x45)
   {
      // must be IP V4 and 20 byte header
	//		syslog(DBG,"recv not ipv4");
      return(0);
   }

      if (//0xff == buf[IP_DST_P ] 
		//	&& 0xff == buf[IP_DST_P+1]
		//	&& 0xff == buf[IP_DST_P+2] 	&& 
		0xff == buf[IP_DST_P+3])
      {
//				syslog(DBG,"recv broadcast:%d.%d.%d.%d:%d",
//					buf[IP_DST_P],buf[IP_DST_P+1],buf[IP_DST_P+2],buf[IP_DST_P+3],
//					(buf[UDP_DST_PORT_H_P]<<8 | buf[UDP_DST_PORT_L_P]));
         return(1);
      }
   ...
   while (i < 4)
   {
      ...
      {
			//	syslog(DBG,"recv ip chk err:%d.%d.%d.%d",buf[IP_DST_P],buf[IP_DST_P+1],buf[IP_DST_P+2],buf[IP_DST_P+3]);
         return(0);
      }
      i++;
   }
...
//      i++;
//   }
	u8PortH	= buf[TCP_SRC_PORT_H_P];
	u8PortL	= buf[TCP_SRC_PORT_L_P];
	buf[TCP_SRC_PORT_H_P] = buf[TCP_DST_PORT_H_P];
   	buf[TCP_SRC_PORT_L_P] = buf[TCP_DST_PORT_L_P];

	 buf[TCP_DST_PORT_H_P] = u8PortH;
	 buf[TCP_DST_PORT_L_P] = u8PortL;
...
void make_udp_reply_from_request(unsigned char* buf, char* data, unsigned char datalen, unsigned  int port)
{
   ...
   make_ip(buf);
   buf[UDP_DST_PORT_H_P] = buf[UDP_SRC_PORT_H_P];//port >> 8;
   buf[UDP_DST_PORT_L_P] = buf[UDP_SRC_PORT_L_P];//port & 0xff;
